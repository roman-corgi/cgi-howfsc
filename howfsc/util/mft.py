...
def do_mft(e, outshape, pixperlod, pixperpupil, direction='forward'):
    """
    ...
    Keyword arguments:
     direction: string, either "forward" or "reverse".  Indicates whether the
     MFT goes to a focus located downstream ("forward", the default) or
     upstream of the pupil.  The two fields will be 180 degrees rotated from
     one another.
    ...
    # Take to image plane
    rowmult = np.exp(sign*2.*np.pi*1j*np.outer(rowpp, row))
    colmult = np.exp(sign*2.*np.pi*1j*np.outer(col, colpp))
...
def do_offcenter_mft(e, outshape, pixperlod, pixperpupil, pupilShape,
    ...
    # Take to image plane
    rowmult = np.exp(sign*2.*np.pi*1j*np.outer(rowpp, rowcrop))
    colmult = np.exp(sign*2.*np.pi*1j*np.outer(colcrop, colpp))

    return np.dot(np.dot(rowmult, e), colmult)*drow*dcol


def do_imft(e, outshape, pixperlod, pixperpupil, direction='forward'):
    """
    ...
    Keyword arguments:
     direction: string, either "forward" or "reverse". Indicates whether the
     IMFT goes to a pupil located upstream ("forward", the default) or
     downstream of the focus. The two fields will be 180 degrees rotated from
     one another. Note that the forward and backward directions mean the
     opposite compared to the MFT because an IMFT propagates the opposite
     direction of an MFT.
    ...
    if direction not in ['forward', 'reverse']:
        raise ValueError('direction must be "forward" or "reverse"')
    if direction == 'forward':
        sign = 1 # opposite sign definitions from MFT
    else: # reverse
        sign = -1
