...
def dmhtoph(nrow, ncol, dmin, nact, inf_func, ppact_d, ppact_cx, ppact_cy, dx,
    ...
    NOTE: this convention--(0,0) in lower-left, and x/y along columns/rows
    respectively--is a choice.  The corner where the (0,0) actuator of a 48x48
    DM pattern falls and x/y directions will differ between DMs (in CGI, DM1
    and DM2 follow each other and so have a parity flip) and potentially
    between instrument and models (as cgisim does not capture the same flips
    as the instrument software and hardware).

    However, as thact + flipx handle the 8 basic 90-degree-rotation-and-flip
    combinations, and (x, y) lateral shift is applied after the flip and
    rotation, the direction of the dx/dy shift applied by dmhtoph is always
    ...
    return insertinto(surface, (nrow, ncol))



def dmh_to_volts(gainmap, dmh, lam, crosstalk_fn=None):
    """
    ...
    Arguments:
     ...
     crosstalk_fn: (optional) string filename to a crosstalk yaml. The
      is as specified in dm_crosstalk.py and is used to create a CDMCrosstalk
      object.
    ...
    # if crosstalk_fn is None, dmxtalk defaults to identity
    dmxtalk = CDMCrosstalk(crosstalk_fn)

    # apply crosstalk to dmh, then apply gain to return Volts
    dmh_command = dmxtalk.crosstalk_backward(dmh)
    dmV_command = dmh_command*lam/(2.0*np.pi)/gainmap

    return dmV_command


def volts_to_dmh(gainmap, volts, lam, crosstalk_fn=None):
    """
    ...
    Arguments:
     ...
     volts: a 2D array of voltages, of the same size as ``gainmap``.
    ...
    # crosstalk is applied to height, first apply gain
    dmh_command = volts/lam*(2.0*np.pi)*gainmap
    dmh_actual = dmxtalk.crosstalk_forward(dmh_command)
