...
class TestDoMFT_IMFT(unittest.TestCase):
    ...
    def test_direction_mft(self):
        """Check scaling type valid in do_imft()."""
        for direction in [-1.5, -1, 0, 1j, [], 'perr', (5,)]:
            with self.assertRaises(ValueError):
                do_mft(self.e, self.outshape, self.pixperlod,
                        self.pixperpupil, direction=direction)
                pass
            pass
        pass


    def test_direction_imft(self):
        """Check scaling type valid in do_imft()."""
        for direction in [-1.5, -1, 0, 1j, [], 'perr', (5,)]:
            with self.assertRaises(ValueError):
                do_imft(self.e, self.outshape, self.pixperlod,
                        self.pixperpupil, direction=direction)
                pass
            pass
        pass
    ...
    def test_reverse_mft_vs_mft(self):
        """
        Test that both MFT directions are the same to within rotation
        """

        tol = 1e-13

        # make nptse odd so we can just use rot90
        d = 31
        outshape = (d, d)
        pixperlod = 5
        nptse = d*pixperlod
        ...
        e = np.zeros((nptse, nptse), dtype=np.complex128)
        # big F, asymmetric
        e[10:140, 50] += 1
        e[70, 50:120] += 1j
        e[140, 50:120] += -1.1

        e_mft_fwd = do_mft(e, outshape, pixperlod, pixperpupil,
                       direction='forward')
        e_mft_rev = do_mft(e, outshape, pixperlod, pixperpupil,
                       direction='reverse')

        # asymmetry means regular won't match, but rotation will
        self.assertFalse(np.max(np.abs(e_mft_fwd - e_mft_rev)) < tol)
        self.assertTrue(np.max(np.abs(e_mft_fwd - np.rot90(e_mft_rev, 2)))
                        < tol)
        pass


    def test_reverse_imft_vs_imft(self):
        """
        Test that both MFT directions are the same to within rotation
        """
        ...
        e_imft_fwd = do_imft(e, outshape, pixperlod, pixperpupil,
                       direction='forward')
        e_imft_rev = do_imft(e, outshape, pixperlod, pixperpupil,
                       direction='reverse')

        # asymmetry means regular won't match, but rotation will
        self.assertFalse(np.max(np.abs(e_imft_fwd - e_imft_rev)) < tol)
        self.assertTrue(np.max(np.abs(e_imft_fwd - np.rot90(e_imft_rev, 2)))
                        < tol)
        pass
...
class TestDoOffcenterMFT(unittest.TestCase):
    ...
    def test_direction_offcenter_mft(self):
        """Check coordinate vector format valid in do_offcenter_mft()."""
        for direction in [(6,), (6, 6, 6), [], None, 6, 'fft']:
            with self.assertRaises(ValueError):
                do_offcenter_mft(self.e, self.outshape, self.pixperlod,
                                 self.pixperpupil, self.pupilShape,
                                 self.yxLowerLeft, direction=direction)


    def test_offcenter_vs_centered_mft_forward(self):
        """Check that offcenter MFT gives same result as a centered MFT."""
        e_on = do_mft(self.efull, self.outshape, self.pixperlod,
                      self.pixperpupil, direction='forward')
        e_off = do_offcenter_mft(self.e, self.outshape, self.pixperlod,
                                 self.pixperpupil, self.pupilShape,
                                 self.yxLowerLeft, direction='forward')

        abs_tol = 100*np.finfo(float).eps
        self.assertTrue(np.max(np.abs(e_on - e_off)) < abs_tol)


    def test_offcenter_vs_centered_mft_reverse(self):
        """Check that offcenter MFT gives same result as a centered MFT."""
        e_on = do_mft(self.efull, self.outshape, self.pixperlod,
                      self.pixperpupil, direction='reverse')
        e_off = do_offcenter_mft(self.e, self.outshape, self.pixperlod,
                                 self.pixperpupil, self.pupilShape,
                                 self.yxLowerLeft, direction='reverse')
