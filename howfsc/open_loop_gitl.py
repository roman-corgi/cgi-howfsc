...
def howfsc_computation_open_loop(framelist, dm1_list, dm2_list, cfg, jac, jtwj_map,
    ...
    try:
        ...
        out = _main_howfsc_computation_open_loop(framelist, dm1_list, dm2_list,
                                       cfg, jac, jtwj_map,
                                       croplist, prev_exptime_list,
                                       cstrat, n2clist, hconf, iteration)
...
def _main_howfsc_computation_open_loop(framelist, dm1_list, dm2_list, cfg, jac, jtwj_map,
                                      croplist, prev_exptime_list,
                                      cstrat, n2clist, hconf, iteration):
    """
    ...
    # NOTE - here is where you get the probe phases from the model
    for n in range(nprobepair):
        ...
        for j in range(nlam):
            ...
            _, tmpph = probe_ap(cfg,
                    ...
                    dm2_list[j*ndm],
                    j)
            # probe_ap returns a tuple of (probe phase, probe amplitude)
    ...
    log.info('Open loop sensing mode: skip control strategy solve')

    # 6. Compute change in DM settings from electric fields
    log.info('6. Compute change in DM settings from electric fields')
    log.info('Open loop sensing mode: skip jac_solve and inv_to_dm')

    # 7. Compute expected mean total contrast for next iteration
    log.info('7. Compute expected mean total contrast for next iteration')

    # TODO - Do we need to adjust the probe scale or height in here?
    # 8. Compute absolute DM settings, camera settings, probe scale factors
    log.info('8. Compute absolute DM settings, camera settings, probe ' +
                 'scale factors')
    log.info('Open loop sensing mode: keep absolute DM settings unchanged')
    ...
    log.info('Get next contrast using current absolute DM settings')
